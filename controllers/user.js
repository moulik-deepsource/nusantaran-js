        ...
        if (error) {
            ...
            const email = {
                ...
                html: Email.getEmail({
                    ...
                    emailText:
                        "Hello, you can reset your password by clicking the button down below. Please notice that this link is only valid for one hour after you recieved this email. Thank you.",
            ...
            try {
                ...
                req.flash(
                    "message",
                    "Email has been sent, token is valid for one hour, please check your email"
                )
                ...
                try {
                    ...
                    req.flash(
                        "message",
                        "Expiration token has been updated to 1 hour, please check your email"
                    )
                    res.status(200).redirect("/reset")
                } catch (error) {
                    next(error)
...
exports.getMyOrder = ash(async (req, res) => {
    ...
    const orderFunction = isAdmin
        ? Users.getAllOrders()
        : Users.getOrdersByEmail(req.session.user.email)
...
exports.getInvoice = ash(async (req, res) => {
    ...
    const invoiceName = sanitize(
        "invoice-" + Date.now() + "-" + orderId + ".pdf"
    )
    ...
    for (let product of JSON.parse(order.product)) {
        pdf.fontSize(16).text(
            `${product.name} (Rp. ${product.price}) x ${product.qty}`
        )
    }
    pdf.end()
})
