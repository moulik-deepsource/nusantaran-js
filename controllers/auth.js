...
exports.postRegister = ash(async (req, res) => {
    ...
    const email = {
        ...
        html: Email.getEmail({
            ...
            emailText:
                "Welcome to Nusantaran. Your account is successfully registered to Nusantaran. Please verify your account by clicking the button down below. Please notice that this link is only valid for 1 hour start from this email is sent. Thank you",
...
exports.postLogin = ash(async (req, res) => {
    ...
    if (!user.verified) {
        req.flash(
            "errorMessage",
            "User is not verified. Check your email and verify your account"
        )
        req.flash("placeholder", req.body.email)
        req.flash("errors", { param: "email" })
        res.redirect("/login")
